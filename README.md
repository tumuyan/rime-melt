...
2. 删除解压后others目录中不必要的文件。
    * `lua`目录是lua滤镜，直接复制到`Rime用户文件夹`内即可
...
更新：2026-06-20 20:50:03
- `pinyin_simp_wiki.dict.yaml`：使用维基百科提供的dump数据解析而得。和别人作品不同，包含了大量中英文混拼词条、含标点的词条。不定期更新。由于维基词条主要还是名词，效用有限，因此使用了一些自制工具对词条进行清理，目前由[Wiki Filter](https://github.com/tumuyan/WikiFilter) 的ci生成。  
[下载直链](https://github.com/tumuyan/rime-pinyin-simp/raw/master/pinyin_simp_wiki.dict.yaml)
- `pinyin_simp_moe.dict.yaml`：使用萌娘百科提供的API数据解析而得，含大量二次元词条。和别人作品不同，包含了大量中英文混拼词条、含标点的词条。不定期更新。    
